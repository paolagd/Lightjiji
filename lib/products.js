...
const getProductsSatisfying = filters => {
  ...
  if (filters.userId) {
    ...
    FROM Products
    LEFT JOIN Favourites ON Products.id = Favourites.product_id AND Favourites.user_id = $${queryParams.length}`;
  }
  ...
  if (filters.searchTerm) {
    ((filters.userId && queryParams.length === 1) || queryParams.length === 0 ) ? queryString += ` WHERE` : queryString += ` AND`;
    queryParams.push(`%${filters.searchTerm.toLowerCase()}%`);
    queryString += ` (LOWER(name) LIKE $${queryParams.length} OR LOWER(description) LIKE $${queryParams.length})`;
  }
